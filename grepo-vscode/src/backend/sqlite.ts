/**
 * Read-only SQLite reader for the grepo database.
 *
...
try {
  ...
  if (isAbiMismatch) {
    console.error(
      '[grepo] better-sqlite3 ABI mismatch. '
      + 'Your VS Code uses a different Node.js version than the one '
      + 'this extension was built for. '
      + 'Try: cd ~/.vscode/extensions/grepo-* && npm rebuild better-sqlite3'
    );
  }
  throw err;
}
...
export class SqliteReader {
  ...
  constructor(dbPath: string) {
    ...
    // Don't set journal_mode here — it's a write operation that fails on
    // readonly connections. The Python side already opened the DB in WAL mode.
    ...
    try {
      ...
      if (tableNames.has('metadata')) {
        ...
        if (row) {
          ...
          // Must match LATEST_VERSION in grepo/migrations.py
  ...
  private _db(): DatabaseType.Database {
    if (!this.db) {
