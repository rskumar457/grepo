...
export class CliWrapper {
    ...
    async embedGraph(workspaceRoot: string): Promise<CliResult> {
        return vscode.window.withProgress(
            {
                location: vscode.ProgressLocation.Notification,
                title: 'Grepo: Computing embeddings\u2026',
                cancellable: false,
            },
    ...
    /**
     * Install the `grepo` package using the specified installer.
     */
    async installBackend(installer: 'uv' | 'pipx' | 'pip'): Promise<CliResult> {
        const commandMap: Record<typeof installer, { bin: string; args: string[] }> = {
            uv: { bin: 'uv', args: ['pip', 'install', 'grepo'] },
            pipx: { bin: 'pipx', args: ['install', 'grepo'] },
            pip: { bin: 'pip3', args: ['install', 'grepo'] },
        };
        ...
        return vscode.window.withProgress(
            {
                location: vscode.ProgressLocation.Notification,
                title: `Grepo: Installing via ${installer}\u2026`,
                cancellable: false,
            },
            async () => {
                try {
    ...
    private getCliPath(): string {
        const configured = vscode.workspace
            .getConfiguration('grepo')
            .get<string>('cliPath', '');
        return configured || 'grepo';
    }
...
function toCliResult(err: unknown): CliResult {
    ...
    if (isEnoent(err)) {
        return {
            success: false,
            stdout: '',
            stderr: 'CLI binary not found. Is grepo installed?',
        };
    }
