 ...
 * Checks `.grepo/graph.db` first, then falls back to `.grepo.db`.
 */
function findGraphDb(workspaceRoot: string): string | undefined {
  const primary = path.join(workspaceRoot, ".grepo", "graph.db");
  if (fs.existsSync(primary)) {
    return primary;
  }

  const fallback = path.join(workspaceRoot, ".grepo.db");
...
function registerCommands(
    ...
    vscode.commands.registerCommand(
      ...
      async () => {
        ...
        if (result.success) {
          ...
          vscode.window.showInformationMessage("Grepo: Build complete.");
        } else {
          vscode.window.showErrorMessage(
            `Grepo: Build failed. ${result.stderr}`
          );
        }
      }
    )
      ...
      async () => {
        ...
        await vscode.window.withProgress(
          ...
          async () => {
            ...
            if (!result.success) {
              vscode.window.showErrorMessage(
                `Grepo: Update failed. ${result.stderr}`
              );
            }
          }
        );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "grepo.showBlastRadius",
      async (qualifiedNameOrUri?: string | vscode.Uri) => {
        if (!sqliteReader) {
          vscode.window.showWarningMessage(
            "Grepo: No graph database loaded."
          );
          return;
        }
        ...
        if (!node) {
          vscode.window.showInformationMessage(
            `Grepo: Node "${qualifiedName}" not found.`
          );
          return;
        }

        const config = vscode.workspace.getConfiguration("grepo");
        ...
        if (impact.impactedNodes.length === 0) {
          vscode.window.showInformationMessage(
            `Grepo: No blast radius found for "${qualifiedName}".`
          );
          return;
        }

        await vscode.commands.executeCommand(
          "grepo.blastRadius.focus"
        );
      }
    )
  );
    ...
    vscode.commands.registerCommand(
      ...
      async (qualifiedName?: string) => {
        ...
        if (callerEdges.length === 0) {
          vscode.window.showInformationMessage(
            `Grepo: No callers found for "${qualifiedName}".`
          );
          return;
        }
    ...
    vscode.commands.registerCommand(
      ...
      async (qualifiedName?: string) => {
        ...
        if (allTestQualifiedNames.size === 0) {
          vscode.window.showInformationMessage(
            `Grepo: No tests found for "${qualifiedName}".`
          );
          return;
        }
  ...
  // -----------------------------------------------------------------
  // grepo.queryGraph — expose all 8 query patterns
  // -----------------------------------------------------------------
  context.subscriptions.push(
    vscode.commands.registerCommand("grepo.queryGraph", async () => {
      if (!sqliteReader) {
        vscode.window.showWarningMessage("Grepo: No graph database loaded.");
        return;
      }
      ...
      if (!node) {
        vscode.window.showInformationMessage(`Grepo: "${target}" not found.`);
        return;
      }
      ...
      if (filtered.length === 0) {
        vscode.window.showInformationMessage(
          `Grepo: No ${pattern.label} results for "${node.name}".`
        );
        return;
      }
    ...
    vscode.commands.registerCommand(
      ...
      async (qualifiedName?: string) => {
        ...
        if (calleeEdges.length === 0) {
          vscode.window.showInformationMessage(
            `Grepo: No callees found for "${qualifiedName}".`
          );
          return;
        }
    ...
    vscode.commands.registerCommand(
      ...
      async () => {
        ...
        if (isNaN(minLines) || minLines < 1) {
          vscode.window.showWarningMessage("Grepo: Invalid line count.");
          return;
        }
        ...
        if (results.length === 0) {
          vscode.window.showInformationMessage(
            `Grepo: No functions found with ${minLines}+ lines.`
          );
          return;
        }
  ...
  // -----------------------------------------------------------------
  context.subscriptions.push(
    vscode.commands.registerCommand("grepo.embedGraph", async () => {
      const workspaceRoot = getWorkspaceRoot();
      if (!workspaceRoot) {
        vscode.window.showErrorMessage("No workspace folder is open.");
        return;
      ...
      if (result.success) {
        vscode.window.showInformationMessage("Grepo: Embeddings computed.");
      } else {
        const msg = result.stderr.includes("not installed")
          ? "Install embeddings support: pip install grepo[embeddings]"
          : `Embedding failed: ${result.stderr}`;
        vscode.window.showErrorMessage(`Grepo: ${msg}`);
      }
    })
  );
  ...
  // -----------------------------------------------------------------
  context.subscriptions.push(
    vscode.commands.registerCommand("grepo.watchGraph", async () => {
      const workspaceRoot = getWorkspaceRoot();
      if (!workspaceRoot) {
        vscode.window.showErrorMessage("No workspace folder is open.");
        return;
      ...
      vscode.window.showInformationMessage("Grepo: Watch mode started.");
      const result = await cli.watchGraph(workspaceRoot);
      if (!result.success) {
        vscode.window.showErrorMessage(
          `Grepo: Watch failed. ${result.stderr}`
        );
      }
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("grepo.showGraph", async () => {
      if (!sqliteReader) {
        vscode.window.showWarningMessage(
          "Grepo: No graph database loaded. Run 'Grepo: Build Graph' first."
        );
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("grepo.search", async () => {
      if (!sqliteReader) {
        vscode.window.showWarningMessage(
          "Grepo: No graph database loaded."
        );
      ...
      if (results.length === 0) {
        vscode.window.showInformationMessage(
          `Grepo: No results found for "${query}".`
        );
        return;
      }
...
function watchGraphDb(context: vscode.ExtensionContext): void {
  ...
  watcher.onDidChange(() => {
    ...
    if (sqliteReader && dbPathRef.current) {
      ...
      vscode.commands.executeCommand("grepo.codeGraph.refresh");
    }
  });
    ...
    if (dbPath) {
      ...
      if (schemaWarning) {
        ...
        if (choice === "Rebuild Graph") {
          await vscode.commands.executeCommand("grepo.buildGraph");
        }
      }

      // Register tree view providers
      const codeGraphProvider = new GrepoTreeProvider(
        sqliteReader,
        workspaceRoot
      );
