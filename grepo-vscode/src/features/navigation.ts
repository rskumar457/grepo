...
export function registerNavigationCommands(
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('grepo.findCallers', async () => {
            ...
            if (!node) {
                vscode.window.showWarningMessage(
                    'Grepo: No graph node found at the current cursor position.',
                );
                return;
            }
            ...
            if (callerEdges.length === 0) {
                vscode.window.showInformationMessage(
                    `Grepo: No callers found for "${node.name}".`,
                );
                return;
            }
    ...
    // -----------------------------------------------------------------
    context.subscriptions.push(
        vscode.commands.registerCommand('grepo.findTests', async () => {
            const reader = getReader();
            if (!reader) {
                vscode.window.showWarningMessage('Grepo: No graph database loaded.');
                return;
            ...
            if (testQualifiedNames.size === 0) {
                vscode.window.showInformationMessage(
                    `Grepo: No tests found for "${node.name}".`,
                );
                return;
            }
