...
export class GraphWebviewPanel {
  ...
  private async openFileAtLine(
    ...
    try {
      ...
      vscode.window.showWarningMessage(
        `Grepo: Could not open file ${filePath}`
      );
    }
  }
  ...
  private async savePngToFile(dataUrl: string): Promise<void> {
    ...
    vscode.window.showInformationMessage("Grepo: PNG saved.");
  }
...
<body>
  ...
  <div id="toolbar">
    ...
    <span id="truncation-warning" style="display:none;color:var(--btn-bg);font-size:11px;margin-left:8px;cursor:pointer;" title="Increase grepo.graph.maxNodes in settings"></span>
  </div>
