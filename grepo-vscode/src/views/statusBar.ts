 ...
 *
 * Clicking the status bar item triggers `grepo.updateGraph`.
 */
export class StatusBar implements vscode.Disposable {
    ...
    constructor() {
        ...
        this.item.command = 'grepo.updateGraph';
    }
    ...
    update(reader: SqliteReader | undefined): void {
        if (!reader) {
            this.item.text = '$(warning) Grepo: Not built';
        ...
        if (isOutdated) {
            this.item.text = '$(warning) Grepo: Outdated';
            this.item.tooltip =
                `Grepo: ${stats.filesCount} files, ${stats.totalEdges} edges\n` +
