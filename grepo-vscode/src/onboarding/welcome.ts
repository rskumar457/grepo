...
export function registerWalkthroughCommands(
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand(
            'grepo.walkthrough.explore',
            async () => {
                await vscode.commands.executeCommand(
                    'grepo.codeGraph.focus',
                );
            },
        ),
    );
 ...
 *
 * Checks for `.grepo/graph.db` in every workspace folder.
...
export async function showWelcomeIfNeeded(
    ...
    const selection = await vscode.window.showInformationMessage(
        'Welcome to Grepo! Get started by building your code graph.',
        'Get Started',
    );
    ...
    if (selection === 'Get Started') {
        await vscode.commands.executeCommand(
            ...
            'rskumar457.grepo#grepo.welcome',
        );
    }
}
