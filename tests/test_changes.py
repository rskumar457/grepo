...
class TestChanges:
    ...
    def test_detect_changes_tool_no_changes(self):
        ...
        from grepo.tools import detect_changes_func

        # Patch _get_store to use our test store,
        # and get_changed_files/get_staged_and_unstaged to return empty.
        with (
            patch("grepo.tools.review._get_store") as mock_get_store,
            patch("grepo.tools.review.get_changed_files", return_value=[]),
            patch("grepo.tools.review.get_staged_and_unstaged", return_value=[]),
        ):
    ...
    def test_detect_changes_tool_with_changes(self):
        ...
        with (
            patch("grepo.tools.review._get_store") as mock_get_store,
            patch("grepo.tools.review.get_changed_files", return_value=["app.py"]),
            patch(
                "grepo.tools.review.parse_git_diff_ranges",
