...
class TestInstallPlatformConfigs:
    def test_install_cursor_config(self, tmp_path):
        ...
        assert "grepo" in data["mcpServers"]
        assert data["mcpServers"]["grepo"]["type"] == "stdio"
    ...
    def test_install_windsurf_config(self, tmp_path):
        ...
        data = json.loads(config_path.read_text())
        entry = data["mcpServers"]["grepo"]
        assert "type" not in entry
        import shutil
        expected_cmd = "uvx" if shutil.which("uvx") else "grepo"
    ...
    def test_install_zed_config(self, tmp_path):
        ...
        assert "grepo" in data["context_servers"]
    ...
    def test_install_continue_config(self, tmp_path):
        ...
        assert data["mcpServers"][0]["name"] == "grepo"
    ...
    def test_continue_array_no_duplicate(self, tmp_path):
        ...
        existing = {
            "mcpServers": [
                {"name": "grepo", "command": "uvx", "args": ["serve"]}
            ]
        }
