...
Manages a registry of multiple repositories at ``~/.grepo/registry.json``
...
_REGISTRY_DIR = Path.home() / ".grepo"
_REGISTRY_PATH = _REGISTRY_DIR / "registry.json"


class Registry:
    """Manages a JSON-based registry of grepo repositories.

    Each entry stores the repo path and an optional alias.
    The registry lives at ``~/.grepo/registry.json``.
    """
    ...
    def register(self, path: str, alias: str | None = None) -> dict[str, str]:
        ...
        Validates that the path contains a ``.git`` or ``.grepo``
        directory.
        ...
        if not (resolved / ".git").exists() and not (resolved / ".grepo").exists():
            raise ValueError(
                f"Path does not look like a repository "
                f"(no .git or .grepo): {resolved}"
            )
