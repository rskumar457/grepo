...
Usage:
    ...
    grepo visualize
    grepo wiki
    grepo detect-changes [--base BASE] [--brief]
    grepo register <path> [--alias name]
    grepo unregister <path_or_alias>
    grepo repos
"""
...
if sys.version_info < (3, 10):
    print("grepo requires Python 3.10 or higher.")
...
{c}  ●──●──●{r}       {d}smarter code reviews{r}
  ...
  {d}Run{r} {b}grepo <command> --help{r} {d}for details{r}
""")
...
def _handle_init(args: argparse.Namespace) -> None:
    ...
    print()
    print("Next steps:")
    print("  1. grepo build    # build the knowledge graph")
...
def main() -> None:
    ...
    if args.command == "eval":
        ...
        if getattr(args, "report", False):
            ...
            print("Run 'grepo eval --report' to generate tables.")
        return
    ...
    if args.command in ("register", "unregister", "repos"):
        ...
        if args.command == "register":
            ...
            if not repos:
                ...
                print("Use: grepo register <path> [--alias name]")
            else:
                for entry in repos:
    ...
    try:
        if args.command == "build":
            ...
            html_path = repo_root / ".grepo" / "graph.html"
