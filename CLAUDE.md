...
**grepo** is a persistent, incrementally-updated knowledge graph for token-efficient code reviews with Claude Code. It parses codebases using Tree-sitter, builds a structural graph in SQLite, and exposes it via MCP tools and prompts.

## Graph Tool Usage (Token-Efficient)
When using grepo MCP tools, follow these rules:
...
- **Core Package**: `grepo/` (Python 3.10+)
...
- **VS Code Extension**: `grepo-vscode/` (TypeScript)
  - Separate subproject with its own `package.json`, `tsconfig.json`
  - Reads from `.grepo/graph.db` via SQLite

- **Database**: `.grepo/graph.db` (SQLite, WAL mode)
...
uv run ruff check grepo/                    # Lint
uv run mypy grepo/ --ignore-missing-imports --no-strict-optional

# Build & test
uv run grepo build                          # Full graph build
uv run grepo update                         # Incremental update
uv run grepo status                         # Show stats
uv run grepo serve                          # Start MCP server
uv run grepo wiki                           # Generate markdown wiki
uv run grepo detect-changes                 # Risk-scored change analysis
uv run grepo register <path>                # Register repo in multi-repo registry
uv run grepo repos                          # List registered repos
uv run grepo eval                           # Run evaluation benchmarks
```
