...
## v1.7.0
- **`install` command**: New primary entry point for setup (`grepo install`). `init` remains as an alias.
...
- **File organization**: Generated files moved into `.grepo/` directory (auto-created `.gitignore`, legacy migration)
...
- **Interactive D3.js graph visualization**: `grepo visualize` generates an HTML graph you can explore in-browser
...
- **Universal install**: `pip install grepo` — no git clone needed
- **CLI entry point**: `grepo` command available system-wide after pip install
...
## v1.1.0
- **Watch mode**: `grepo watch` — auto-rebuilds graph on file changes
...
- Graph stored in `.grepo/graph.db` (SQLite), auto-gitignored
- No telemetry, no network calls
- Respects `.gitignore` and `.grepoignore`
