...
```
Requires: `pip install grepo[embeddings]`
...
```bash
# Setup
grepo install           # Register MCP server with Claude Code (alias: init)
grepo install --dry-run # Preview without writing files

# Build and update
grepo build                        # Full build
grepo update                       # Incremental update
grepo update --base origin/main    # Custom base ref

# Monitor and inspect
grepo status                       # Graph statistics
grepo watch                        # Auto-update on file changes
grepo visualize                    # Generate interactive HTML graph

# Analysis
grepo detect-changes               # Risk-scored change analysis
grepo detect-changes --base HEAD~3 # Custom base ref
grepo detect-changes --brief       # Compact output

# Wiki
grepo wiki                         # Generate markdown wiki from communities

# Multi-repo
grepo register <path> [--alias name]  # Register a repository
grepo unregister <path_or_alias>       # Remove from registry
grepo repos                            # List registered repositories

# Evaluation
grepo eval                         # Run evaluation benchmarks

# Server
grepo serve                        # Start MCP server (stdio)
```
