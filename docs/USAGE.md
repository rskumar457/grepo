...
```bash
pip install grepo
grepo install    # auto-detects and configures all supported platforms
grepo build      # parse your codebase
```
...
```bash
grepo install --platform cursor
grepo install --platform claude-code
```
...
grepo wiki
```
Creates markdown wiki pages for each detected community in `.grepo/wiki/`.

### 10. Multi-repo search (v2)
```bash
grepo register /path/to/other/repo --alias mylib
```
...
To add custom patterns, create a `.grepoignore` file in your repo root (same syntax as `.gitignore`):
